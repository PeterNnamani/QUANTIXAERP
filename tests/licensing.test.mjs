...
import { getPlanUserLimit, getSubscriptionReplacementStatus, getTrialEndDate, isTrialActive, planCanAccessRoute, planHasFeature, TRIAL_PLAN } from '../lib/licensing.ts'
...
test('plan routes enforce the advertised tier boundary', () => {
    assert.equal(planCanAccessRoute('Growth Edition', '/asset-schedule'), true)
    ...
    assert.equal(planCanAccessRoute('Growth Edition', '/product-manager'), true)
})
