...
import { applyBankAccountSave } from '@/lib/bank-account-save'
...
export default function SettingsPage() {
  ...
  const handleCreateBank = () => {
    ...
    const previousName = state.bankAccounts.find((account) => account.id === saved.account?.id)?.name
    const renamed = saved.updated && previousName && previousName !== saved.account.name
    updateState({
      banks: saved.banks,
    ...
    resetBankForm()
    setBankCreationStatus(saved.updated ? `${accountKey} was updated and Bank Balances now shows the changes.` : `${accountKey} was created and is now available in Bank Balances.`)
  }
          ...
          <div className="settings-content">
              ...
              <div className="panel-card">
                <div className="panel-head">
                  <div>
                    ...
                    <div className="panel-subtitle">These balances are stored on the chart of accounts and show on the ledger and management accounts.</div>
                  </div>
            ...
            {activeSection === 'banks' && (
              <div className="panel-card">
                ...
                <div className="panel-subtitle">{editingBankId ? 'Changes save on this bank and update the Bank Balances cards.' : 'Add an account here and it will appear immediately on the Bank Balances page.'}</div>
                <div className="form-grid two-up">
                  ...
                  {state.bankAccounts.map((account) => (
                    <div className="bank-register-row" key={account.id}>
                      <span>
                        ...
                        <small>{account.accountNumber || 'No account number'}</small>
                        <button className="action-btn bank-row-edit" type="button" onClick={() => startEditBank(account)}>Edit</button>
                      </span>
                      <span>{account.accountType}</span>
