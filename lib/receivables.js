export function buildReceivableFromSale(sale) {
    ...
    const isCredit = String(sale.paymentStatus || '').toUpperCase() === 'CREDIT'
    if (!isCredit) {
        return {
            id: `AR-${sale.id}`,
...
export function mergeReceivablesFromSales(sales, existingReceivables = []) {
    ...
    const nextBySale = new Map()
    for (const sale of sales) {
        if (String(sale.paymentStatus || '').toUpperCase() !== 'CREDIT') continue
